...
texaxis=r"""
                \begin{axis}[
                        ...
                        xmax=10^7,
                        xlabel={Percentiles [\%]},
                        ylabel={Relative difference [\si{\percent}]},
...
# latency histogram
hist = sorted(list(measurement_folder.glob('**/*.pcap.reldiff.csv')))
for f in hist:
    filname = f.stem.replace('.reldiff', '')
    fhandle = open(plot_folder.joinpath(filname + '.reldiff.tex'), 'w')
